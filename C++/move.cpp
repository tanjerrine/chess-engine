...
vector<Move> knights_legal_moves(U64 knight_poss, Board* board) {
    vector<Move> moves;
    char color = board->get_turn();
    // Captures
    U64 opp_pieces = (color == 'w' ? board->get_b_pieces() : board->get_w_pieces());
    U64 east_caps = opp_pieces & (color == 'w' ? w_pawn_east_atks(pawn_poss) : b_pawn_east_atks(pawn_poss));
    U64 west_caps = opp_pieces & (color == 'w' ? w_pawn_west_atks(pawn_poss) : b_pawn_west_atks(pawn_poss));
    while (east_caps) {
        U64 curr = east_caps & -east_caps;
        U64 start_pos = (color == 'w' ? sw_one(curr) : nw_one(curr));
        Move m(0, start_pos, curr, true, curr & en_passant);
